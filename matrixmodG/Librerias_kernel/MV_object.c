/*---------------------------------------------------------------------------------------------------------------------------------------------------------
 * Libreria MV_object.c (archivo fuente - source file).
 * 
 * 
...
extern void new_matrix(matrix_o *p_mo, char *p_name)
{
	...
	{
		/* Primero asigno metodos a la vtable del objeto para que conozca todos sus metodos disponibles. */
		...
		p_mo->columnas= 0;
		p_mo->tam_bytes= 0;
		p_mo->realTam_bytes= 0;
		p_mo->mc= OBJ_NO_CREATED;
		p_mo->alloc_mode= M_KZALLOC;
	...
	{
		if(MATRIXMODG_DB_MSG) printk(KERN_ERR "matrixmod_error: Error al inicializar nuevo componente, el componente %s ya fue inicializado !!!\n", p_mo->name);
	}
}
...
{	
	...
	size_t aux_i = 0;
	size_t aux_tam = 0;
	size_t aux_realTam = 0;
	short alloc_fail = 0;


	if(p_mo->matriz==NULL){

		if (p_mo->alloc_mode == M_KMALLOC) /* Si modo de resrva de memoria es M_KMALLOC: Realizo la reserva de memoria dinamica mediante kmalloc().*/
		{
			/* Reserva de memoria para filas de m (creación de arreglo de punteros) */
			m = (int **)kmalloc(p_mo->filas*sizeof(int*), GFP_KERNEL);
			...
			for (i=0; i<p_mo->filas; i++) {
				m[i] = (int*)kmalloc(p_mo->columnas*sizeof(int), GFP_KERNEL);
				...
				aux_realTam = aux_realTam + ksize(m[i]);
			}
		...
		else if (p_mo->alloc_mode == M_KZALLOC) /* Si modo de resrva de memoria es M_KZALLOC: Realizo la reserva de memoria dinamica mediante kmalloc() y kzalloc().*/
		{
			/* Reserva de memoria para filas de m (creación de arreglo de punteros) */
			m = (int **)kmalloc(p_mo->filas*sizeof(int*), GFP_KERNEL);
			if(m == NULL) /* Si falla la reserva.*/
			...
			/* Reserva de memoria para columnas de m (creacion de arreglos de enteros por cada m fila) */
			for (i=0; i<p_mo->filas; i++) {
				m[i] = (int*)kzalloc(p_mo->columnas*sizeof(int), GFP_KERNEL);
				if(m[i] == NULL) /* Si falla la reserva en arreglos de alguna fila.*/
				{
					alloc_fail = 1;
					aux_i = i; /* rescato el valor de i, para liberar lo previamente asignado.*/
		...
		else if (p_mo->alloc_mode == M_KCALLOC) /* Si modo de resrva de memoria es M_KCALLOC: Realizo la reserva de memoria dinamica mediante kmalloc_array() y kcalloc().*/
		{
			/* Reserva de memoria para filas de m (creación de arreglo de punteros) */
			m = (int **)kmalloc_array(p_mo->filas, sizeof(int*), GFP_KERNEL);
			if(m == NULL) /* Si falla la reserva.*/
			{
				return ALLOC_ERROR;
			}
			...
			/* Reserva de memoria para columnas de m (creacion de arreglos de enteros por cada m fila) */
			for (i=0; i<p_mo->filas; i++) {
				m[i] = (int*)kcalloc(p_mo->columnas, sizeof(int), GFP_KERNEL);
				if(m[i] == NULL) /* Si falla la reserva en arreglos de alguna fila.*/
				{
					alloc_fail = 1;
					aux_i = i; /* rescato el valor de i, para liberar lo previamente asignado.*/
		...
		else if(p_mo->alloc_mode == M_VMALLOC) /* Si modo de resrva de memoria es M_VMALLOC: Realizo la reserva de memoria dinamica mediante vmalloc().*/
		{
			/* Reserva de memoria para filas de m (creación de arreglo de punteros) */
			m = (int **)vmalloc(p_mo->filas*sizeof(int*));
			if(m == NULL) /* Si falla la reserva.*/
			{
				return ALLOC_ERROR;
			}
			...
			aux_tam = p_mo->filas*sizeof(int*);
			aux_realTam = p_mo->filas*sizeof(int*);
			
			/* Reserva de memoria para columnas de m (creacion de arreglos de enteros por cada m fila) */
			for (i=0; i<p_mo->filas; i++) {
				m[i] = (int*)vmalloc(p_mo->columnas*sizeof(int));
				if(m[i] == NULL) /* Si falla la reserva en arreglos de alguna fila.*/
				{
					alloc_fail = 1;
					aux_i = i; /* rescato el valor de i, para liberar lo previamente asignado.*/
				...
				aux_tam = aux_tam + p_mo->columnas*sizeof(int);
				aux_realTam = aux_realTam + p_mo->columnas*sizeof(int);
			}
		...
		else if(p_mo->alloc_mode == M_VZALLOC) /* Si modo de resrva de memoria es M_VZALLOC: Realizo la reserva de memoria dinamica mediante vmalloc() y vzalloc().*/
		{
			/* Reserva de memoria para filas de m (creación de arreglo de punteros) */
			m = (int **)vmalloc(p_mo->filas*sizeof(int*));
			if(m == NULL) /* Si falla la reserva.*/
			...
			/* Reserva de memoria para columnas de m (creacion de arreglos de enteros por cada m fila) */
			for (i=0; i<p_mo->filas; i++) {
				m[i] = (int*)vzalloc(p_mo->columnas*sizeof(int));
				if(m[i] == NULL) /* Si falla la reserva en arreglos de alguna fila.*/
				{
					alloc_fail = 1;
					aux_i = i; /* rescato el valor de i, para liberar lo previamente asignado.*/
		...
		{
			if(MATRIXMODG_DB_MSG) printk(KERN_ERR "matrixmod_error: No se pudo reservar memoria. El componente %s no tiene asignado un modo valido de reserva de memoria!!!\n", p_mo->name);
		...
		p_mo->realTam_bytes = aux_realTam; 	/* Guardo registro de cantidad de memoria real reservada en la matriz del objeto matrix_o. */
...
{	
	...
	if(p_mo->matriz != NULL){
		...
		if( (p_mo->alloc_mode == M_VMALLOC) || (p_mo->alloc_mode == M_VZALLOC) ) /* Si modo de resrva de memoria es de la familia M_VMALLOC: Realizo la liberacion de memoria dinamica mediante vfree().*/
		{
		...
		else if ( (p_mo->alloc_mode == M_KMALLOC) || (p_mo->alloc_mode == M_KZALLOC) || (p_mo->alloc_mode == M_KCALLOC) ) /* Si modo de resrva de memoria es  de la familia M_KMALLOC: Realizo la liberacion de memoria dinamica mediante kfree().*/
		{
			/* Libero reserva de memoria para columnas de matriz de p_mo */
			for (i = 0; i<(p_mo->filas); i++) 
				kfree(p_mo->matriz[i]);
		...
		{
			if(MATRIXMODG_DB_MSG) printk(KERN_ERR "matrixmod_error: No se pudo liberar memoria. El componente %s tiene conflictos con el modo de reserva de memoria!!!\n", p_mo->name);
...
void set_Mmem_alloc_mode(matrix_o *p_mo, int p_mode)
{
	...
	if(p_mo->mc == OBJ_NO_CREATED) 
	{
		...
		{	
			/* Si el modo ingresado no es valido se asigna por defecto M_KMALLOC.*/
			p_mo->alloc_mode = M_KMALLOC;	
		}
	}
	else
	{
		if(MATRIXMODG_DB_MSG) printk(KERN_ERR "matrixmod_error: No es posible cambiar el modo de reserva de memoria para el componente %s, el componente ya esta creado !!!\n", p_mo->name);
	}
}
...
static int valid_alloc_mode(int p_mode)
{
	...
	if(MATRIXMODG_DB_MSG) printk(KERN_ERR "matrixmod_error: Se detecto un modo de reserva de memoria no valido por la libreria de objetos matrix_o.\n");
...
void rollback_alloc(int **p_m, size_t pos_fail, int p_mode)
{
	...
	if( (p_mode == M_VMALLOC) || (p_mode == M_VZALLOC) )
	{
		/*
		 * Libero reserva de memoria en parte de columnas de matriz de p_mo, antes de pos_fail posicion deonde
	...
	else if( (p_mode == M_KMALLOC) || (p_mode == M_KZALLOC) || (p_mode == M_KCALLOC) )
	{
		/*
		 * Libero reserva de memoria en parte de columnas de matriz de p_mo, antes de pos_fail posicion deonde
		 * se produce la falla de asignacion de memoria.
	...
	{
		if(MATRIXMODG_DB_MSG) printk(KERN_ERR "matrixmod_error: No se pudo liberar memoria. Conflictos de asignacion y fallo la liberacion rollback_alloc !!!\n");
	}
}
...
extern int crear_matriz_cero(matrix_o *p_mo, int p_filas, int p_columnas)
{
    ...
    if( (p_filas > 0) && (p_columnas > 0) && (p_mo->mc != OBJ_CREATED)){
        ...
        {
	        if(MATRIXMODG_DB_MSG){
	        	printk(KERN_INFO "matrixmod_info: Se reservo exitosamente memoria para %d filas de %s!!!\n", p_filas, p_mo->name);
				printk(KERN_INFO "matrixmod_info: Se reservo exitosamente memoria para %d columnas de %s!!!\n", p_columnas, p_mo->name);
			}
...
int crear_matriz_identidad(matrix_o *p_mo, int p_dimension)
{
    ...
    if( (p_dimension > 0) && (p_mo->mc != OBJ_CREATED)){
        ...
        if(p_mo->mo_methods->alloc_matrix_mem(p_mo) == ALLOC_OK)
        {
	        ...
	        if(MATRIXMODG_DB_MSG){ 
	        	printk(KERN_INFO "matrixmod_info: Se reservo exitosamente memoria para %d filas de componente %s!!!\n", p_dimension, p_mo->name);
			 	printk(KERN_INFO "matrixmod_info: Se reservo exitosamente memoria para %d columnas de componente %s!!!\n", p_dimension, p_mo->name);
			 }
...
void negar_matriz(matrix_o *p_mo)
{
	...
	if((p_mo->filas > 0) && (p_mo->columnas > 0)) {
		...
		if(MATRIXMODG_DB_MSG) printk(KERN_INFO "matrixmod_error: No se puede negar matriz %s.\n", p_mo->name);
}
...
void new_vector(vector_o *p_vo, char * p_name)
{
	if(p_vo->mc != OBJ_CREATED) /* Si objeto vector_o p_vo no fue creado previamente. Se inicializan sus campos.*/
	{
		...
		p_vo->elements= 0;
		p_vo->tam_bytes= 0;
		p_vo->realTam_bytes= 0;
		p_vo->mc= OBJ_NO_CREATED;
		p_vo->alloc_mode= M_KZALLOC;
...
{	
	...
	if(p_vo->vector==NULL){
		...
		if(p_vo->alloc_mode == M_VMALLOC) /* Si modo de resrva de memoria es M_VMALLOC: Realizo la reserva de memoria dinamica mediante vmalloc().*/
		{
			/* Reserva de memoria para elementos de p_vector (creacion de arreglo de enteros) */
			p_vector = (int *)vmalloc(p_vo->elements*sizeof(int));
			...
			/* Con memoria virtual el tamaño real es igual al tamaño asignado. */
			aux_realTam = p_vo->elements*sizeof(int);

			/* Rellena cada elemento del vector con cero. */
			rellenar_vector(p_vector, p_vo->elements, 0);

		}
		else if(p_vo->alloc_mode == M_VZALLOC) /* Si modo de resrva de memoria es M_VZALLOC: Realizo la reserva de memoria dinamica mediante vzalloc().*/
		{
			/* Reserva de memoria para elementos de p_vector (creacion de arreglo de enteros) */
			p_vector = (int *)vzalloc(p_vo->elements*sizeof(int));
		...
		else if (p_vo->alloc_mode == M_KMALLOC) /* Si modo de resrva de memoria es M_KMALLOC: Realizo la reserva de memoria dinamica mediante kmalloc().*/
		{
			/* Reserva de memoria para elementos de p_vector (creacion de arreglo de enteros) */
			p_vector = (int *)kmalloc(p_vo->elements*sizeof(int), GFP_KERNEL);
			...
			aux_realTam = ksize(p_vector);

			/* Rellena cada elemento del vector con cero. */
			rellenar_vector(p_vector, p_vo->elements, 0);
		}
		else if ((p_vo->alloc_mode == M_KZALLOC) || (p_vo->alloc_mode == M_KCALLOC)) /* Si modo de resrva de memoria es M_KZALLOC o M_kCALLOC: Realizo la reserva de memoria dinamica mediante kzalloc().*/
		{
			/* Reserva de memoria para elementos de p_vector (creacion de arreglo de enteros) */
			p_vector = (int *)kzalloc(p_vo->elements*sizeof(int), GFP_KERNEL);
		...
		p_vo->realTam_bytes = aux_realTam; /* Guardo registro de cantidad de memoria real reservada en el vector del objeto vector_o. */
...
int free_vector_mem(vector_o *p_vo)
{
	...
	if(p_vo->vector != NULL){
		...
		if( (p_vo->alloc_mode == M_VMALLOC) || (p_vo->alloc_mode == M_VZALLOC) ) /* Si modo de resrva de memoria es de la familia M_VMALLOC: Realizo la liberacion de memoria dinamica mediante vfree().*/
		{
			/* Liberar memoria para elementos de vector de p_vo */
			vfree(p_vo->vector);
		}
		else if ( (p_vo->alloc_mode == M_KMALLOC) || (p_vo->alloc_mode == M_KZALLOC) || (p_vo->alloc_mode == M_KCALLOC) )  /*Si modo de resrva de memoria es  de la familia M_KMALLOC: Realizo la liberacion de memoria dinamica mediante kfree().*/
		{
			/* Liberar memoria para filas de matriz de p_mo */
			kfree(p_vo->vector);
		}
...
extern int crear_vector_cero(vector_o *p_vo, int p_elements)
{
    ...
    if( (p_elements > 0) && (p_vo->mc != OBJ_CREATED)){
        ...
        {
	        if(MATRIXMODG_DB_MSG) printk(KERN_INFO "matrixmod_info: Se reservo exitosamente memoria para %d elementos de %s!!!\n", p_elements, p_vo->name);
...
void negar_vector(vector_o *p_vo)
{
	...
	if(p_vo->vector != NULL) {
		...
		if(MATRIXMODG_DB_MSG) printk(KERN_INFO "matrixmod_error: No se puede negar vector %s.\n", p_vo->name);
}
